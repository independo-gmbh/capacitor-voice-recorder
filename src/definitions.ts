...
export interface CurrentRecordingStatus {
  /**
   * The current status of the recorder, which can be one of the following values: 'RECORDING', 'PAUSED', 'INTERRUPTED', 'ERROR', 'NONE'.
   *
   * `'ERROR'` means the session died while it was running, see
   * `VoiceRecordingFailedEvent`. It is deliberately not `'NONE'`: there is
   * still a file to collect, and `stopRecording()` will hand back whatever was
   * captured before the failure.
   */
  status: 'RECORDING' | 'PAUSED' | 'INTERRUPTED' | 'ERROR' | 'NONE';
}
 ...
 */
/**
 * A recording session that died while it was running: the media server went
 * away, the microphone was taken, the encoder failed.
 *
 * Distinct from every other error this plugin reports, which all come back
 * through the call that caused them. Nothing is in flight when this happens, so
 * without this event nobody finds out: the recorder simply stops producing
 * audio while the app goes on believing it is recording.
 */
export type VoiceRecordingFailedEvent = {
  /**
   * Machine-readable. Android: `RECORDER_SERVER_DIED`, `RECORDER_UNKNOWN_ERROR`,
   * `AMPLITUDE_READ_FAILED`, `INTERRUPTION_FAILED`. iOS:
   * `RECORDER_STOPPED_UNEXPECTEDLY`.
   */
  code: string;
  /** Whatever the platform said. Diagnostic, not user-facing on its own. */
  message?: string;
};
...
export interface VoiceRecorderPlugin {
  ...
  /**
   * Fail the running session on purpose, so `recordingFailed` fires and the
   * app can rehearse what it does about it.
   *
   * For testing on a real device, where the alternative is waiting for a media
   * server to die. Not a simulation of the *report*: the session really does
   * end up in `'ERROR'`, with its metering stopped and (on Android) its file
   * left unfinalized, which is the part worth rehearsing.
   *
   * Rejects with `RECORDING_HAS_NOT_STARTED` when nothing is recording.
   */
  simulateRecordingFailure(): Promise<GenericResponse>;
  ...
  /**
   * Listen for a recording session dying while it runs, see
   * `VoiceRecordingFailedEvent`.
   *
   * On iOS this rides on the volume metering timer, so it only fires when
   * `volumeMetering` is on; Android reports it either way.
   *
   * @param eventName The name of the event to listen for.
   * @param listenerFunc The callback function to invoke when the event occurs.
   * @returns A promise that resolves to a PluginListenerHandle.
   */
  addListener(
    eventName: 'recordingFailed',
    listenerFunc: (event: VoiceRecordingFailedEvent) => void,
  ): Promise<PluginListenerHandle>;
